					...
					for (var/datum/data/record/R in data_core.security)
						if (R.fields["id"] == E.fields["id"])
							if(hasHUD(usr,"security"))
								...
								if(ishuman(usr))
									var/mob/living/carbon/human/U = usr
									R.fields[text("com_[counter]")] = text("Made by [U.get_authentification_name()] ([U.get_assignment()]) on [addtext(time2text(world.realtime, "DDD MMM DD"), " ", time2text(world.timeofday, "hh:mm:ss"))], [game_year]<BR>[t1]")
								if(isrobot(usr))
									var/mob/living/silicon/robot/U = usr
									R.fields[text("com_[counter]")] = text("Made by [U.name] ([U.modtype] [U.braintype]) on [addtext(time2text(world.realtime, "DDD MMM DD"), " ", time2text(world.timeofday, "hh:mm:ss"))], [game_year]<BR>[t1]")
