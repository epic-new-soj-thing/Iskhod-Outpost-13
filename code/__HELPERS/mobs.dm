...
/proc/able_mobs_in_oview(var/range, var/origin)
	var/list/mobs = list()
	var/list/view_contents = oview(range, origin) // Cache the result

	// Process living mobs
	for(var/mob/living/M in view_contents)
		if(!M.is_physically_disabled())
			mobs += M

	// Process mechas and their pilots
	for(var/obj/mecha/potential_mech in view_contents)
		var/mob/living/M = potential_mech.get_mob()
		if(M && !M.is_physically_disabled() && !(M in mobs)) // Prevent duplicates
			mobs += M
